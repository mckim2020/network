...
from sreconstruct import Reconstruct
...
class Extractor:
    ...
    def reconstruct(self):
        ...
        if self.config.extract.use_teasar:
            ...
            self.net = core.Network(network_data)
        else:
            self.net = Reconstruct.get_network(self.image_stack, scale=self.xy_spacing,
    ...
    def __show_step(self, name, binary=False, spacing=None):
        if self.logger.level <= logging.DEBUG:
            ...
            # Render.show_3d_image(self.image_stack, name, binary=binary, spacing=spacing) # Temporarily disabled
