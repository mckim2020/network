...
# import Qiber3D
import shelper as helper
from srender import Render
...
class Reconstruct:  # BJH
    ...
    def __add_segment_from_path(cls, net, graph, path):
        ...
        net.segment[new_id] = core.Segment(radius=np.array(new_radius), point=np.array(new_points),
    ...
    def __join_segments(cls, net, segment_ids):
        ...
        new_points, new_radius = cls.__combine_radius_points(new_points, add_points, new_radius, add_radius)
        net.segment[new_id] = core.Segment(radius=np.array(new_radius), point=np.array(new_points), segment_index=new_id)
    ...
    def smooth(cls, net, voxel_per_point=10):
        ...
        for sid, segment in net.segment.items():
            ...
            r[r < 1] = 1 # NEED TO LOOK INTO IT LATER!
            segment.radius = r

        for point, segment_ids in cpd.items():
            cls.__join_branch_point(net, point, segment_ids)
        
        # SAVE SEGMENT DATA - BEFORE SMOOTHING
        with open('segments_smooth.pkl', 'wb') as f:
            pickle.dump(net.segment, f)
            print("SMOOTHED SEGMENT DATA SAVED.")
    ...
    def create_base_network(cls, image):
        ...
        # result = skeletonize(image, method='lee') # Alternative method
        ...
        ax[1].imshow(result[10, :, :], cmap=plt.cm.gray)
        ax[1].axis('off')
        ax[1].set_title('skeleton', fontsize=20)
        fig.tight_layout()
        # plt.savefig('skeleton_example.png')
        # print("SKELETON IMAGE SAVED.")



        # RENDERING - using vedo
        # if cls.logger.level <= 10:
        # Render.show_3d_image(result, name='Skeleton', binary=True)
        ...
        graph = nx.Graph()
        volume_shape = image.shape # Z, X, Y pixel numbers
        if cls.logger.level <= 10:
        ...
        fromStack = np.zeros([1, 3])
        toStack = np.zeros([1, 3])
        for direction, correction, start_slice, end_slice in search_slices:
            ...
            # Concatenate
            fromStack = np.concatenate((fromStack, from_points), axis=0)
            toStack = np.concatenate((toStack, to_points), axis=0)
        ...
        # # SAVE FROM-TO POINTS
        # print("Size of the nodes (start point): ", fromStack.shape)
        # print("Size of the nodes (end point): ", toStack.shape)
        # np.save('./fromStack.npy', fromStack)
        # np.save('./toStack.npy', toStack)
        ...
        segment_data = IO.IO.load._from_graph(graph, point_lookup=volume_shape,
                                                   radius_lookup=distances, ravel=True)
        
        # # SAVE SEGMENT DATA - BEFORE SMOOTHING
        # with open('segments_raw.pkl', 'wb') as f:
        #     pickle.dump(segment_data, f)
        #     print("RAW SEGMENT DATA SAVED.")

        data = {
            'path': None,
            'name': 'reconstruct',
            'segments': segment_data}
        net = core.Network(data)
        return net
    ...
    def get_network(cls, image, scale=1.0, input_path=None):
        ...
        cls.clean(net)
        net = IO.IO.load.network(net)
        cls.smooth(net)
        net = IO.IO.load.network(net, scale=scale, input_path=input_path)
        return net
